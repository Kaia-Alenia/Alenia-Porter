...
export default function App() {
  ...
  const [appVersion, setAppVersion] = useState("6.8");
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (pendingFiles.length > 0) {
      ...
      addLog(t("errorNoFiles") || "No compatible file (Video/Audio/Image) detected.", "error");
      setMascotState("error");
    }
  };
  ...
  const handleManualFileInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (pendingFiles.length > 0) {
        ...
        addLog(t("errorNoCompatible", "No compatible file for Alenia Porter formats."), "error");
      }
    }
  };
  ...
  const handleExplainCommand = async () => {
    ...
    try {
      ...
      if (pywebview && pywebview.api && pywebview.api.explain_command) {
        ...
        setMascotState("winking");
        addLog(t("explanationGenerated", "Command explanation generated by local analyzer"), "success");
      } else {
        throw new Error(t("pyAnalyzerDisconnected", "The local Python analyzer is not connected."));
      }
    } catch (err: any) {
      console.error(err);
...
GPU Encoder Detected: ${gpuEncoderDetected === "none" ? t("none", "None") : gpuEncoderDetected}
...
=====================================================`;
  ...
  const startBatchQueueIndex = (index: number) => {
    ...
    if (cacheKey && cachedOptimizations[cacheKey] && !(window as any).pywebview) {
      ...
      addLog(t("cacheMatch", "[Smart-Cache] MD5/SHA fingerprint match in local cache!"), "success");
      addLog(`${t("cacheSkip", "[Smart-Cache] Skipping redundant compilation for: ")}${file.name}`, "success");
    ...
    addLog(isGpuActive ? `[FFmpeg-WASM] GPU Activo: ${gpuEncoderDetected === "none" ? t("none", "None") : gpuEncoderDetected}` : `[FFmpeg-WASM] Using soft encoding (Inactive drivers / Safe Mode)`, "info");
            ...
            <div className="space-y-3">
                ...
                >
                  {activeMediaType === "video" && (
                    <>
                      ...
                      <optgroup label={t("optgroup_otros", "── Otros ──")}>
                        ...
                        <option value="3gp">{t("opt_3gp", "3GP (Mobile)")}</option>
                        <option value="gif">{t("opt_gif", "GIF (Animation)")}</option>
                      </optgroup>
                    </>
                  )}
                  {activeMediaType === "audio" && (
                    <>
                      <optgroup label={t("optgroup_sinprdida", "── Lossless ──")}>
                      ...
                      <optgroup label={t("optgroup_conprdida", "── Lossy ──")}>
                        ...
                        <option value="amr">{t("opt_amr", "AMR (Mobile)")}</option>
                      </optgroup>
                  ...
                  {activeMediaType === "image" && (
                    <>
                      ...
                      <optgroup label={t("optgroup_estndar", "── Standard ──")}>
              ...
              {filesQueue.some(f => f.isDirectory) && (
                <div className="flex flex-col gap-1 p-2 rounded-lg border border-dashed" style={{ borderColor: "rgba(99,102,241,0.4)", background: "rgba(99,102,241,0.04)" }}>
                  ...
                  <span className={`text-[7.5px] font-mono ${THEME_COLORS[themeColor].textMuted}`}>{t("selectTabFormat", "Select each tab to configure its format")}</span>
                </div>
              )}
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    <option value="libx264">{t("opt_libx264", "H.264 / AVC (libx264)")}</option>
                      ...
                      <option value="libsvtav1">{t("opt_libsvtav1", "AV1 (SVT-AV1, fast)")}</option>
        ...
        {showSettings && (
          <div className={`absolute inset-0 ${THEME_COLORS[themeColor].bodyBg} z-45 flex flex-col animate-in slide-in-from-bottom duration-300 transition-colors`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {/* SECTION 4: PERFORMANCE AND DIAGNOSTICS */}
              <div className={`${THEME_COLORS[themeColor].cardBg} border ${THEME_COLORS[themeColor].cardBorder} rounded-2xl p-4 shadow-2xs space-y-3 transition-colors`}>
                <div className={`flex items-center gap-2 border-b ${THEME_COLORS[themeColor].cardBorder} pb-2`}>
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <span className={`text-[8px] ${THEME_COLORS[themeColor].textMuted} font-mono`}>{t("hwDetectedLabel")}: {gpuEncoderDetected === "none" ? t("none", "None") : gpuEncoderDetected}</span>
                    </div>
              ...
              {/* SECTION 5: PRIVACY AND COMMUNITY TELEMETRY */}
              <div className={`${THEME_COLORS[themeColor].cardBg} border ${THEME_COLORS[themeColor].cardBorder} rounded-2xl p-4 shadow-2xs space-y-3 transition-colors`}>
                <div className={`flex items-center gap-2 border-b ${THEME_COLORS[themeColor].cardBorder} pb-2`}>
