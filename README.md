# Alenia Porter v3.0

### Media Optimizer for Game Engines (Ren'Py / Godot)

Alenia Porter is a tool designed to automate the conversion and optimization of multimedia assets, ensuring that final files are compatible and lightweight for independent projects.

---

## Supported Languages

* 🇲🇽 Español - Translated by KXLT - Alenia Studios
* 🇺🇸 English - Translated by KXLT - Alenia Studios
* 🇫🇷 Français - Translated by KXLT - Alenia Studios
* 🇯🇵 日本語 - Translated by KXLT - Alenia Studios
* 🇨🇳 简体中文 - Translated by KXLT - Alenia Studios

### Community Contributors (Translations)
Your name here. If you have contributed a translation for Alenia Porter, add your name to this list in your Pull Request:
1. [Space available for new contributors]

---

## Contributions and Development

We invite the community to improve Alenia Porter. You can collaborate on:
* Interface (UI): Improve design and user experience.
* Logic: Optimize conversion processes and file handling.
* Languages: If you want to see Alenia Porter in your language, you can edit the locales.json file and submit your changes.

### Roadmap
We are currently working on extending support to Unity and other engines. Due to our current workstation specifications (8GB of RAM), testing on heavier engines is done progressively.

---

## License and Legal Terms

This project is distributed under the Standard Licensing: Alenia Studios Standard (CC BY 4.0 + Additional Terms).

Conditions of Use and Attribution:
1. Shared Attribution: Mandatory credit must be given to Alenia Studios for the creation of the tool and to the person who did the translation to the language being used in the final project.
2. Commercial Use: Allowed for the creation of games, videos, and commercial projects.
3. Prohibition of Resale: Independent redistribution, sublicensing, or resale of this tool is prohibited. You may not charge for the tool or its usage.
4. No AI: Explicitly prohibits the use of this code, documentation, or assets for training Artificial Intelligence models or inclusion in datasets.

---
Developed by Alenia Studios.
Black and purple.